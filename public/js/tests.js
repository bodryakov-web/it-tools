...
function handleTestAnswer(event) {
    ...
    const questionElement = document.querySelector(`[data-test-question="${questionNumber}"]`);
    ...
    const allAnswersInQuestion = questionElement.querySelectorAll('[data-test-answer]');
...
function isQuestionAnswered(questionNumber) {
    const questionElement = document.querySelector(`[data-test-question="${questionNumber}"]`);
    return questionElement && questionElement.classList.contains('test-question--answered');
}
...
function getTestStatistics() {
    ...
    questions.forEach(question => {
        if (question.classList.contains('test-question--answered')) {
            ...
            if (correctAnswer && correctAnswer.classList.contains('test-answer--correct')) {
                correctAnswers++;
            }
        }
    });
...
function resetAllTests() {
    ...
    questions.forEach(question => {
        ...
        answers.forEach(answer => {
            ...
            answer.classList.remove('test-answer--disabled', 'test-answer--correct', 'test-answer--incorrect');
            
            const marker = answer.querySelector('.test-answer__marker');
            if (marker) {
                marker.innerHTML = '';
                marker.classList.remove('test-answer__marker--correct', 'test-answer__marker--incorrect');
            }
        });
    });
}